...
bar_chart(
    ...
    "share of output tokens removed (higher is better) · Termux/aarch64 · whitespace metric from rtk's own tests",
    [
    ...
    100, (25, 50, 75, 100), "%",
    note="* command unknown to rtk — rtk passes it through raw; tok's generic filter compresses anything",
)

bar_chart(
    "bench2.svg",
    "Token savings — rtk's own test fixtures",
    "end-to-end replay of fixtures from the rtk repo (gradle / maven / glab — its flagship modules) · higher is better",
    [
...
bar_chart(
    "latency.svg",
    "Latency — Termux/aarch64",
    "mean of 30 runs after warm-up · lower is better",
    [
        ("startup (--version)", *ms(11.1, 9.2)),
        ("git status end-to-end", *ms(41.5, 25.5)),
        ("ls (large directory)", *ms(17.9, 10.0)),
        ("PreToolUse hook", *ms(51.0, 10.0)),
    ],
    55, (10, 20, 30, 40, 50), " ms",
    note="the PreToolUse hook fires on every single Bash call — tok is 5.1x faster there (no SQLite, no config parse)",
)
...
bar_chart(
    ...
    'find src -name "*.rs" called three times · tokens emitted · rtk has no such feature',
    [
        ("1st call", 66, "66 tokens", 41, "41 tokens"),
        ("2nd call (no changes)", 66, "66 tokens", 7, '7 — "unchanged since last run"'),
        ("3rd call (1 file added)", 66, "66 tokens", 10, "~10 — one-line diff only"),
    ],
    70, (20, 40, 60), "",
)
...
bar_chart(
    ...
    [
        ("Bash result bytes", 100, "2794 B", 89.6, "2504 B"),
        ("tokens (whitespace)", 100, "357", 88.0, "314"),
        ("tokens (est. BPE)", 100, "1122", 89.4, "1003"),
        ("task wall-time", 100, "97.2 s", 83.9, "81.5 s"),
    ],
    100, (25, 50, 75, 100), "%",
    note="both agents succeeded: 4/4 tests PASS + commit · measured from the agents' transcripts",
)
